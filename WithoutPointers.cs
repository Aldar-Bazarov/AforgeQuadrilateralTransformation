...
namespace TogiSoft.AtlasDataBase.ArchiveWell.Perspective.Quadrilateral
{
    ...
    internal class QuadrilateralTransformation
    {
        ...
        {
            ColorsData = new Color[image.Height * image.Width];
            IntData = new int[image.Height * image.Width];

            for (int i = 0; i < image.Width; i++)
            {
                for (int j = 0; j < image.Height; j++)
                {
                    ColorsData[i + j] = image.GetPixel(i, j);
                }
            }
            ...
            DoPerspective(new UnmanagedImage(image), new UnmanagedImage(image));

            for (int i = 0; i < ColorsData.Length; i++)
            {
                ColorsData[i] = Color.FromArgb(IntData[i]);
                //IntData[i] = ColorsData[i].ToArgb();
            }
            ...
            for (int i = 0; i < image.Width; i++)
            {
                for (int j = 0; j < image.Height; j++)
                {
                    image.SetPixel(i, j, ColorsData[i + j]);
                }
            }
        ...
        protected unsafe void DoPerspective(UnmanagedImage sourceData, UnmanagedImage destinationData)
        {
            ...
            //var ptr = (byte*)destinationData.ImageData;
            //var baseSrc = (byte*)sourceData.ImageData;
            ...
            for (int y = 0; y < dstHeight; y++)
            {
                for (int x = 0; x < dstWidth; x++)
                {
                    ...
                    if ((srcX >= 0) && (srcY >= 0) && (srcX < srcWidth) && (srcY < srcHeight))
                    {
                        ...
                        //
                        p11 = p22 = baseSrcc + (sy1 * srcStride);
                        p11 += sx1 * pixelSize;
                        p22 += sx2 * pixelSize;

                        p33 = p44 = baseSrcc + (sy2 * srcStride);
                        p33 += sx1 * pixelSize;
                        p44 += sx2 * pixelSize;
                        //

                        for (int i = 0; i < pixelSize; i++, ptrr++, p11++, p22++, p33++, p44++)
                        {
                            IntData[ptrr] = (byte)((dy2 * ((dx2 * (IntData[p11])) + (dx1 * (IntData[p22])))) + (dy1 * ((dx2 * (IntData[p33])) + (dx1 * (IntData[p44])))));
